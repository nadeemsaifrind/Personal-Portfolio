  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto w-full max-w-375 px-4 sm:px-8 lg:px-12">
        ...
        {/* Proof stats — inline, not a separate slab */}
        <motion.div
          className="mt-14 grid grid-cols-3 gap-6 border-t border-border pt-8 max-w-[40rem]"
        ...
        >
          {proofStats.map((stat) => (
            <div key={stat.label}>
              <p className="text-2xl font-black tracking-tightest text-foreground sm:text-3xl">{stat.value}</p>
              <p className="mt-1 text-xs text-foreground/40 leading-snug">{stat.label}</p>
            </div>
          ))}
        </motion.div>
      </div>
