...
export function CaseStudies({ selected: externalSelected, setSelected: externalSetSelected, dialogOnly = false }: Props = {}) {
  ...
  return (
    <>
    {!dialogOnly && <section className="border-t border-border" id="projects">
        ...
        <div className="flex items-end justify-between mb-12">
          <div>
            ...
            <p className="mt-3 text-sm text-muted-foreground max-w-md">
              Real client work, real competition results, real products shipped. Click any case to see the full breakdown.
            </p>
          </div>
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-between mb-2.5">
                  ...
                  <span className="font-mono text-[9px] text-muted-foreground/40">{cs.year}</span>
                </div>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => { if (!o) { setSelected(null); setOpenClient(null); setOpenMediaFolder(null); } }}>
            ...
            <div className="flex flex-col">
              ...
              <div className="p-8 md:p-10 space-y-10">
                    ...
                    >
                      ...
                      <div className="bg-[#e8e8e8] px-4 py-2.5 flex items-center gap-3 border-b border-black/8">
                        ...
                        <div className="flex-1 flex items-center gap-2 bg-white/80 rounded-md px-3 py-1 max-w-xs mx-auto">
                          ...
                          <span className="font-mono text-[10px] text-gray-500 tracking-tight">{selected.platformUrl ?? "app.demo"}</span>
                        </div>
                ...
                <div>
                  ...
                  <p className="text-base text-foreground/80 leading-relaxed">{selected.challenge}</p>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {selected.instagramProof.btsPhotos.map((photo, i) =>
                        photo ? (
                          ...
                          <div key={i} className="rounded-2xl border-2 border-dashed border-border bg-secondary/20 aspect-4/3 flex items-center justify-center">
                            <span className="font-mono text-[9px] text-muted-foreground/30">BTS PHOTO {i + 1}</span>
                          </div>
                        )
                      )}
                    </div>
                        ...
                        <div>
                          ...
                          {client.images.length > 0 ? (
                            ...
                            <div className="rounded-xl border-2 border-dashed border-border bg-secondary/20 h-40 flex flex-col items-center justify-center gap-2">
                              ...
                              <span className="font-mono text-[8px] text-muted-foreground/25">Add images to src/assets/ then import in portfolio-data.ts</span>
                            </div>
                          )}
                        ...
                        {selected.clientGallery.map((client) => (
                          ...
                          >
                            ...
                            <div className="px-3 py-2.5 flex items-center justify-between">
                              ...
                              <span className="text-muted-foreground/30 group-hover:text-foreground/50 transition-colors text-xs">→</span>
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      {openMediaFolder ? (() => {
                        if (openMediaFolder === "events" && selected.eventGallery?.length) {
                          return (
                            <>
                              <p className="font-mono text-[10px] tracking-[0.2em] text-foreground/50 mb-4">EVENTS</p>
                              <div className="columns-2 gap-2.5 space-y-2.5">
                                {selected.eventGallery.map((src, i) => (
                                  <div key={i} className="break-inside-avoid overflow-hidden rounded-xl">
                                    <img src={src} alt={`Event ${i + 1}`} className="w-full block hover:scale-[1.03] transition-transform duration-500" draggable={false} />
                                  </div>
                                ))}
                              </div>
                            </>
                        ...
                        if (openMediaFolder === "success" && selected.successStories?.length) {
                          return (
                            <>
                              <p className="font-mono text-[10px] tracking-[0.2em] text-foreground/50 mb-4">SUCCESS STORIES</p>
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {selected.successStories.map((story, i) => (
                                  <div key={i} className="flex flex-col gap-1.5">
                                    <div className="overflow-hidden rounded-2xl aspect-3/4">
                                      <img src={story.image} alt={story.caption ?? `Story ${i + 1}`} className="w-full h-full object-cover hover:scale-[1.04] transition-transform duration-500" draggable={false} />
                                    </div>
                                    {story.caption && (
                                      <p className="font-mono text-[9px] tracking-[0.12em] text-muted-foreground/60 text-center leading-snug">{story.caption}</p>
                                    )}
                                  </div>
                                ))}
                              </div>
                        ...
                        if (openMediaFolder === "campaigns" && selected.campaignGallery?.length) {
                          return (
                            <>
                              <p className="font-mono text-[10px] tracking-[0.2em] text-foreground/50 mb-4">MARKETING CAMPAIGNS</p>
                              <div className="columns-2 gap-2.5 space-y-2.5">
                                {selected.campaignGallery.map((src, i) => (
                                  <div key={i} className="break-inside-avoid overflow-hidden rounded-xl">
                                    <img src={src} alt={`Campaign ${i + 1}`} className="w-full block hover:scale-[1.03] transition-transform duration-500" draggable={false} />
                                  </div>
                                ))}
                              </div>
                            </>
                          ...
                          {folders.map((folder) => (
                            ...
                            >
                              <div className="h-24 bg-secondary/40 overflow-hidden grid grid-cols-3 gap-px">
                                {[0, 1, 2].map((i) =>
                                  folder.preview[i] ? (
                                    <img key={i} src={folder.preview[i]} alt="" aria-hidden className="w-full h-full object-cover group-hover:scale-[1.06] transition-transform duration-500" />
                                  ) : (
                                    <div key={i} className="w-full h-full bg-border/40" />
                                  )
                                )}
                              ...
                              <div className="px-3 py-2.5 flex items-center justify-between">
                                <span className="font-mono text-[10px] tracking-[0.12em] text-foreground/70">{folder.name.toUpperCase()}</span>
                                <span className="text-muted-foreground/30 group-hover:text-foreground/50 transition-colors text-xs">→</span>
                              </div>
                            </button>
                          ))}
                  ...
                  return (
                    <div>
                      ...
                      <div className="flex items-center gap-2 mb-5">
                        ...
                        <span className="font-mono text-[10px] tracking-[0.2em] text-muted-foreground">INSTAGRAM PRESENCE BUILT</span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="relative overflow-hidden rounded-2xl" style={{ aspectRatio: "9/16" }}>
                          {ig.profileScreen ? (
                            <img src={ig.profileScreen} alt="Instagram profile" className="w-full h-full object-cover object-top hover:scale-[1.03] transition-transform duration-500"/>
                          ) : (
                            <div className="w-full h-full bg-linear-to-b from-gray-100 to-gray-50 flex flex-col items-center justify-center gap-2">
                              <svg className="w-6 h-6 text-[#E1306C]/30" viewBox="0 0 24 24" fill="currentColor">
                              ...
                              <span className="font-mono text-[8px] text-muted-foreground/35">Profile screenshot</span>
                            </div>
                          )}
                          {/* Profile label */}
                          <div className="absolute inset-0 bg-linear-to-t from-black/60 via-transparent to-transparent pointer-events-none"/>
                          <div className="absolute bottom-3 left-3 right-3">
                            <span className="font-mono text-[9px] text-white/80">@livingroomservicedapartments</span>
                          </div>
                        </div>

                        {/* Two reels */}
                        {ig.reels.map((reel, i) => (
                          <div key={i} className="relative overflow-hidden rounded-2xl" style={{ aspectRatio: "9/16" }}>
                            {reel.thumbnail ? (
                              <img src={reel.thumbnail} alt={`Reel ${i+1}`} className="w-full h-full object-cover hover:scale-[1.03] transition-transform duration-500"/>
                            ) : (
                              <div className="w-full h-full bg-linear-to-b from-gray-800 to-gray-950 flex flex-col items-center justify-center gap-2">
                                <svg className="w-7 h-7 text-white/20" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                                <span className="font-mono text-[7px] text-white/25 text-center px-3">cs-livingroom-reel-{i+1}.jpg</span>
                              </div>
                            )}
                            ...
                            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                              <div className="flex items-center gap-1">
                                <svg className="w-3 h-3 text-white/70" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                                <span className="font-mono text-[9px] text-white font-medium">{reel.views}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <svg className="w-3 h-3 text-[#E1306C]" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
                                <span className="font-mono text-[9px] text-white font-medium">{reel.likes}</span>
                              </div>
                            </div>
                          </div>
                        ))}
                ...
                {selected.testimonial && (() => {
                  ...
                  const parts = t.highlightName
                    ? t.quote.split(t.highlightName)
                    : [t.quote];
                  return (
                    <div>
                      <span className="font-mono text-[10px] tracking-[0.2em] text-muted-foreground block mb-3">
                      ...
                      >
                        ...
                        <div className="flex items-start gap-3 px-5 pt-5 pb-4 border-b border-[#0a66c2]/10">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-muted-foreground leading-snug mt-0.5 line-clamp-1">{t.title}</p>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            <span className="font-semibold text-base text-foreground leading-tight">{n.name}</span>
                          ...
                          <p className="text-[11px] text-muted-foreground/60 font-mono tracking-widest mb-1.5">{n.frequency}</p>
                          <p className="text-sm text-muted-foreground leading-snug line-clamp-2">{n.description}</p>
                        </div>
                        {/* Arrow */}
